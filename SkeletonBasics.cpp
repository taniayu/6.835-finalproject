...
void CSkeletonBasics::ProcessSkeleton()
{
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        if (NUI_SKELETON_TRACKED == trackingState)
        {
			...
			trainingData.setDatasetName("KinectData");

			//You can also add some info text about the data
			trainingData.setInfoText("This data contains some skeleton timeseries data");
			...
			VectorDouble pos(trainingData.getNumDimensions());
			pos[0] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].x;
			pos[1] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].y;
			pos[2] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].z;
			pos[3] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].x;
			pos[4] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].y;
			pos[5] = skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_ELBOW_RIGHT].z;
			if (m_bSeatedMode) {
				trainingSample.push_back(pos);
				//trainingData.addSample(1, trainingSample);
				skeletonCount += 1;
			}

			//After recording your training data you can then save it to a file
			if (!m_bSeatedMode && skeletonCount > 0) {
				trainingData.addSample(gestureLabel, trainingSample);
				trainingSample.clear();
				skeletonCount = 0;
				sampleCount += 1;
				//TimeSeriesClassificationData testData = trainingData;
				UINT classLabel = trainingData[0].getClassLabel();
				MatrixDouble timeseries = trainingData[0].getData();
				...
				if (predictedClassLabel == 2) {
					D2D1_POINT_2F start = D2D1::Point2F(20, 20);
					D2D1_POINT_2F end = D2D1::Point2F(20, 100);
					ID2D1SolidColorBrush *vert;
					m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(1, 0, 0), &vert);
					m_pRenderTarget->DrawLine(start, end, vert, 10);
				}
				else if (predictedClassLabel == 1) {
					D2D1_POINT_2F start = D2D1::Point2F(20, 20);
					D2D1_POINT_2F end = D2D1::Point2F(100, 20);
					ID2D1SolidColorBrush *hor;
					m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(1, 0, 0), &hor);
					m_pRenderTarget->DrawLine(start, end, hor, 10);
				}
				...
				/*if (!trainingData.saveDatasetToFile("TrainingData" + std::to_string(sampleCount) + ".txt")) {
			...
			//if (sampleCount == 1) {
			//	if (!trainingData.saveDatasetToFile("TrainingData" + std::to_string(i) + ".txt")) {
			//		cout << "Failed to save dataset to file!\n";
			//		return;
			//	}
			//	//If need you can clear any training data that you have recorded
...
void CSkeletonBasics::SetStatusMessage(const WCHAR* szMessage)
{
			...
			if (SPET_LPARAM_IS_OBJECT == curEvent.elParamType)
			{
				...
				if (SUCCEEDED(hr))
				{
					if ((pPhrase->pProperties != NULL) && (pPhrase->pProperties->pFirstChild != NULL))
					{
						...
						{
							HPAction action = MapSpeechTagToAction(pSemanticTag->pszValue);
							SetStatusMessage(pSemanticTag->pszValue);
							DoAction(action);
						}
					}
...
/// </returns>
HPAction CSkeletonBasics::MapSpeechTagToAction(LPCWSTR pszSpeechTag)
{
	...
	{
		{ L"STUPEFY", StupefySpell },
		{ L"PROTEGO", ProtegoSpell },
		{ L"START", StartGesture },
		{ L"STOP", StopGesture }
	};

	HPAction action = HPActionNone;
...
void CSkeletonBasics::DoAction(HPAction action) {
	switch (action)
	{
	case StupefySpell:
		break;
